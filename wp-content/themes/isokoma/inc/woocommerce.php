...
function wp_commerce_template_single_rating(){
    ...
    if ($product->single_add_to_cart_text() === 'Thêm vào giỏ hàng'){      
			$cart_text = 'Cart';
		}	

    if ('Add to cart' === $product->single_add_to_cart_text() ){      
			$cart_text = 'Cart';
		}
		...
		<div class="woocommerce-product-details__short-description">		
			<?php if (is_user_logged_in()) { 					
				$the_quantity_managing_stock = false;
				if (!$product->managing_stock()) {
					$the_quantity_managing_stock = true;			
				}	
				if (($the_quantity_managing_stock == true)) {
				?>
               ...
               <?php _e($product->single_add_to_cart_text() ? 'Cart' : 'Cart', 'isokoma'); ?></button>
