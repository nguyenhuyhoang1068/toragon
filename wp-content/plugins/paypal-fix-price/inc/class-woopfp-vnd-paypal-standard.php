 ...
 *
 */
class WooPfp_VND_PayPal_Standard {
	...
	/**
	 * WooPfp_VND_PayPal_Standard constructor.
	 *
	...
	public function add_exchange_rate_info( $value ) {
		if ( ! is_admin() ) {
			...
			$value['description'] .=
				sprintf( __( 'The prices will be converted to %1$s in the PayPal pages with the exchange rate %2$s.', 'woo-pfp' ),
	...
	public function remove_currency_and_total_checks() {
		...
		require_once dirname( __FILE__ ) . '/paypal-standard/class-woo-pfp-wc-gateway-paypal-ipn-handler.php';
		$handler = new Woo_Pfp_WC_Gateway_Paypal_IPN_Handler( $testmode, $receiver_email ); //@todo needs to handle the correct variables
