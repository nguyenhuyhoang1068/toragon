...
class WC_Pre_Orders_Manager {
	...
	public function check_for_pre_order_products_to_reset() {
		...
		//	update_post_meta( $post_id, '_rpt_price_original', $product->get_regular_price() );			
		}
	...
	private function disable_pre_orders_for_products( $product_ids ) {
		...
		foreach ( $product_ids as $product_id ) {
			...
			delete_post_meta( $product_id, '_rpt_prices' );			
			delete_post_meta( $product_id, 'rpt_apply_sale' );		
			delete_post_meta( $product_id, 'rpt_new_layout' );		
			update_post_meta( $product_id, '_regular_price', get_post_meta($product_id, '_rpt_price_original', true) );
			update_post_meta( $product_id, '_price', get_post_meta($product_id, '_rpt_price_original', true) );
